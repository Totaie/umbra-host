...
function readThemePreference(): ThemePreference {
  const value = localStorage.getItem('umbra.theme');
...
export const useSystemStore = defineStore('system', () => {
  ...
  const error = ref('');
  const navCollapsed = ref(storedBoolean('umbra.nav-collapsed', false));
  ...
  function setTheme(value: ThemePreference): void {
    theme.value = value;
    localStorage.setItem('umbra.theme', value);
    applyTheme();
  }

  function toggleNav(): void {
    navCollapsed.value = !navCollapsed.value;
    localStorage.setItem('umbra.nav-collapsed', String(navCollapsed.value));
  }
