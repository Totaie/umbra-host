...
namespace playnite_launcher::playnite {
  namespace {
    ...
    constexpr wchar_t cleanup_watchdog_cache_mutex[] = L"Local\\UmbraPlayniteWatchdogCache-v1";
    constexpr wchar_t cleanup_lease_mutex_prefix[] = L"Local\\UmbraPlayniteCleanupLease-v1-";
    ...
    std::optional<std::filesystem::path> cleanup_watchdog_cache_root() {
      ...
      std::filesystem::path root = std::filesystem::path(local_app_data) / L"Umbra" / cleanup_watchdog_cache_directory;
