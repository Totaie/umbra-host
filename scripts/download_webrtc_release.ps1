...
function New-GitHubHeaders {
  ...
  $headers = @{
    ...
    "User-Agent" = "umbra-webrtc-downloader"
...
if (-not $OutDir) {
  $OutDir = if ($env:WEBRTC_ROOT) { $env:WEBRTC_ROOT } else { Join-Path $scriptRoot ".umbra-deps\libwebrtc\out" }
}
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) "umbra-webrtc-$([System.Guid]::NewGuid().ToString('N'))"
