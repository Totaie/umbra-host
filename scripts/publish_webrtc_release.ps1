...
if (-not $releaseExists) {
  ...
  if ($NotesPath -and (Test-Path -LiteralPath $NotesPath)) {
    ...
    $createArgs += @("--notes", "Pinned Windows x64 WebRTC artifacts for Umbra.")
  }
  if ($env:GITHUB_SHA) {
...
if ($existingAsset.Count -gt 0) {
  $tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) "umbra-webrtc-publish-$([System.Guid]::NewGuid().ToString('N'))"
