...
> (Organization → Project `Umbra` → Artifact configurations).

## Why two signing requests ("the whole package, setup and all")

The shipped artifact is a single self-contained `UmbraSetup.exe`. It is a
...
| `setup-exe` | [`setup-exe.artifact-config.xml`](setup-exe.artifact-config.xml) | `ci-windows.yml` (setup-EXE request) | Authenticode-sign the outer `UmbraSetup.exe` |
...
1. confirms the outer `UmbraSetup.exe` is signed,
...
3. Confirm the GitHub trusted-build system is linked to project `Umbra`.
