...
By default the script caches its working tree and output to `%LOCALAPPDATA%\Umbra\deps\libwebrtc\{src,out}`,
which is **shared across every Umbra build dir, worktree, and git checkout on the machine**. This means:
...
- Multiple Umbra build dirs share one cached libwebrtc.

`cmake/dependencies/webrtc.cmake` looks at the same default location, so no `-DWEBRTC_ROOT=...` is needed after the
first build. To relocate the cache, set `UMBRA_DEPS_DIR=<path>` in your environment before invoking either the
script or CMake.
...
7. Stage the artifacts into a directory with `include/` and `lib/` subfolders. Either place them at the default
   shared cache location (`%LOCALAPPDATA%\Umbra\deps\libwebrtc\out`) so CMake finds them automatically, or
